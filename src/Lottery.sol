...
struct Receipt {
    ...
    /// @dev 2**64 - 1 = 18446744073709551615, which should not be reached.
    uint64 poolId;
    /// @notice The starting ticket id this receipt is for.
    /// @dev 2**120 - 1 = 1329227995784915872903807060280344575, should be enough.
    uint120 ticketId;
    /// @notice The number of tickets bought in this receipt.
    /// @dev 2**64 - 1 = 18446744073709551615, which should not be reached.
    uint64 count;
    /// @notice Wether or not this receipt has been used to claim a jackpot.
    bool claimed;
}

/// @notice The four possible settlement states for a lottery pool.
...
contract Lottery is Ownable, VRFV2WrapperConsumerBase {
    ...
    event TicketsBought(address recipient, Receipt receipt);
    ...
    /// @notice Reverted when creating a pool with an invalid cutoff delay.
    /// @param cutoffDelay The cutoffDelay.
    error InvalidCutoffDelay(uint256 cutoffDelay);
    ...
    /// @notice Reverted when trying settle a pool that does not exist.
    /// @param poolId The pool id.
    ...
    /// @notice Reverted when trying reclaim a jackpot with a winning receipt.
    /// @param receiptId The receipt id.
    error AlreadyClaimed(uint256 receiptId);
    ...
    function setVrfCallbackGasLimit(uint32 gasLimit) external onlyOwner {
        ...
        emit VrfCallbackGasLimitUpdated({gasLimit: gasLimit});
    }
    ...
    function setVrfRequestConfirmations(
        ...
        emit VrfRequestConfirmationsUpdated({
    ...
    function createPool(
        ...
        if (ticketPrice < 1e14) {
            revert InvalidTicketPrice({price: ticketPrice});
        }

        // Ensure the non-winner percent can't be equal or more than 100%.
        if (noWinnerPercent >= 1e4) {
            revert InvalidNoWinnerPercent({percent: noWinnerPercent});
        }

        // Ensure the fee percent can't be equal or more than 100%.
        if (feePercent >= 1e4) {
            revert InvalidFeePercent({percent: feePercent});
        }

        // Ensure the cutoff delay is valid.
        uint256 remainingTime = expiry - block.timestamp;
        if (remainingTime < cutoffDelay) {
            revert InvalidCutoffDelay({cutoffDelay: cutoffDelay});
        }
    ...
    function withdrawFee(address recipient, uint256 amount) external onlyOwner {
        ...
        emit FeeWithdrawn({recipient: recipient, amount: amount});
    }
    ...
    function withdrawLink(
        ...
        emit LinkWithdrawn({recipient: recipient, amount: amount});
    }
    ...
    function buyTicketsForPool(
        ...
        // Ensure the pool is not in cutoff period (and the pool exists).
        uint256 remainingTime = pool.expiry - block.timestamp;
        if (remainingTime < pool.cutoffDelay) {
        ...
        Receipt memory receipt = Receipt({
            ...
            // UNSAFE: Casting to uint120 is safe because depositedAmount / ticketPrice should
            // never be that large.
            ticketId: uint120(pool.depositedAmount / ticketPrice),
        ...
        emit TicketsBought({recipient: recipient, receipt: receipt});
    }
    ...
    function intiatePoolSettlement(uint256 poolId) external {
        ...
        if (pool.expiry == 0) {
            revert PoolDoesNotExist({poolId: poolId});
        }
        ...
        uint256 requestId = requestRandomness({
            _callbackGasLimit: vrfCallbackGasLimit,
            _requestConfirmations: vrfRequestConfirmations,
            _numWords: 1
        });
    ...
    /// @dev Reverts if the settlement has not been initiated.
    function finalizePoolSettlement(uint256 poolId) external {
        ...
        // type(uint120).max.
        pool.winningTicketId = uint120(winningTicketId);

        // Compute the fee and increase the available fee reserve.
        uint256 jackpot = pool.startingJackpot + depositedAmount;
    ...
    /// @param receiptId The winning receipt id.
    function claimJackpot(uint256 receiptId) external {
        Receipt memory receipt = receipts[receiptId];
        Pool storage pool = pools[receipt.poolId];

        // Ensure the user is not replaying its winning receipt.
        if (receipt.claimed) {
            revert AlreadyClaimed({receiptId: receiptId});
        }
        ...
        // Consume the receipt.
        receipts[receiptId].claimed = true;
    ...
    function fulfillRandomWords(
        ...
        // NOTE: Save gas by temporarly storing the random number in the winningTicketId field.
        pool.winningTicketId = uint120(randomWords[0]);
